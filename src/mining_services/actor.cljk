(ns mining-services.actor
  "MiningServicesActor — the ISIC-08 0990 mining services contractor
  coordination actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one coordination operation
  ...
  The unconditional invariant: the MiningServicesAdvisor can never
  directly commit a record or dispatch any operation the
  MiningServicesGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mining-services.advisor :as advisor]
            [mining-services.governor :as governor]
            [mining-services.store :as store]))

(defn build-graph
  "Build a compiled MiningServicesActor graph. `store` implements
  `mining-services.store/Store`. `advisor` implements
  `mining-services.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:contractor-id (:contractor-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one coordination operation request to completion or interrupt.
